...
// y3: Y3 coordinate
// color :color
void lcdDrawTriangleByPoints(TFT_t *dev, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t x3, uint16_t y3, uint16_t color)
{
	lcdDrawLine(dev, x1, y1, x2, y2, color);
	lcdDrawLine(dev, x1, y1, x3, y3, color);
	lcdDrawLine(dev, x2, y2, x3, y3, color);
...
// Draws filled bottom flat triangle
// x1: X1 coordinate
// y1: Y1 coordinate
// x2: X2 coordinate
// y2: Y2 coordinate
...
// y3: Y3 coordinate
// color :color
void lcdDrawBottomFlatFilledTriangle(TFT_t *dev, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t x3, uint16_t y3, uint16_t color)
{
	float invslope1 = 0;
	float invslope2 = 0;
	if (y2 != y1)
	...
	for (int scanlineY = y1; scanlineY <= y2; scanlineY++)
	{
		...
		lcdDrawLine(dev, (int)curx1, scanlineY, (int)curx2, scanlineY, color);
		curx1 += invslope1;
		curx2 += invslope2;
	}
}

void lcdDrawTopFlatFilledTriangle(TFT_t *dev, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t x3, uint16_t y3, uint16_t color)
{
	float invslope1 = 0;
	float invslope2 = 0;
	if (y3 != y1)
...
void swapVertices(uint16_t *x1, uint16_t *y1, uint16_t *x2, uint16_t *y2)
{
	uint16_t tmpX = *x1;
	uint16_t tmpY = *y1;
	*x1 = *x2;
...
void sortVerticesAscendingByY(uint16_t *x1, uint16_t *y1, uint16_t *x2, uint16_t *y2, uint16_t *x3, uint16_t *y3)
{
	if (*y3 < *y2)
	{
...
void lcdDrawFilledTriangle(TFT_t *dev, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t x3, uint16_t y3, uint16_t color)
{
	...
	if (y2 == y3)
	{
		lcdDrawBottomFlatFilledTriangle(dev, x1, y1, x2, y2, x3, y3, color);
	}
	/* check for trivial case of top-flat triangle */
	else if (y1 == y2)
	{
		lcdDrawTopFlatFilledTriangle(dev, x1, y1, x2, y2, x3, y3, color);
	}
	else
	{
		/* general case - split the triangle in a topflat and bottom-flat one */
		uint16_t x4 = (int)(x1 + ((float)(y2 - y1) / (float)(y3 - y1)) * (x3 - x1));
		uint16_t y4 = y2;
		lcdDrawBottomFlatFilledTriangle(dev, x1, y1, x2, y2, x4, y4, color);
		lcdDrawTopFlatFilledTriangle(dev, x2, y2, x4, y4, x3, y3, color);
	}
}
